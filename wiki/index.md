...
| [broken-assets.md](broken-assets.md) | The three asset paths the site references that have no file: both Auslandsreisen PDFs linked from `/faqs` (lost in the Webflow scrape, and the English one was earning 6.7k GSC impressions) and `mv-logo.jpg` in twelve pages' client-injected Article schema. Includes the re-runnable audit command. | Any asset move, or before calling an asset-touching change done |
