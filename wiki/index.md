...
| [reviews-pipeline.md](reviews-pipeline.md) | How a review travels from `/neue-bewertung` through the `automations` Worker into `public/reviews.json` and onto the page — and the currently broken link in that chain (the Worker still writes the pre-migration path). | Any work on reviews, the review count/average, or the review schema |
